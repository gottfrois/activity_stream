class CommentPresenter < BasePresenter
  ...
  def metadata(*args)
    ...
    # out << link_to('Like', activity_comment_likes_path(comment.activity, comment), :method => 'post')                          if options[:like]        && comment.respond_to?(:vote) && !comment.voted_by?(current_user)
    # out << link_to('Unlike', activity_comment_likes_path(comment),  :method => 'destroy')                    if options[:like]        && comment.respond_to?(:vote) && comment.voted_by?(current_user)
